...
export class DealWithG41_G42 {
    ...
    private static detect(line: string): boolean {
    ...
    private static validate(line: string): boolean {
    ...
    private static apply(line: string): boolean {
        console.log(" - A P P L Y  G41  O R  G42  C O M A N D ! - ");
        const codeType = line.match(/(G41|G42)/ig);
        const last: number = codeType?.length ? codeType!.length - 1 : 0;
    ...
    private static updateHTML(): boolean {
        return true;
    }
}