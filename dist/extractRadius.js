export class ExtractRadius {
    ...
    static checkForIJKvsRConflict(coord) {
        const ijkCheck = (Math.abs(coord.ijkr.i) > 0.0001 || Math.abs(coord.ijkr.j) > 0.0001 || Math.abs(coord.ijkr.k) > 0.0001);
        const rCheck = (coord.ijkr.r > 0.0001);
        if (ijkCheck && rCheck) {
    ...
    static atLeastOneValueBigEnough(coord) {
        if (Math.abs(coord.ijkr.i) < 0.0001 ||
